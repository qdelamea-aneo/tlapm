...
let hyp_level_info_direct h =
    ...
    `level_computation#hyp` below, but only for the cases where the answer
    does not depend on first re-leveling the hypothesis:
    ...
    - a defined operator (`Defn`) or a fact (`Fact`) whose body already
      carries level information can be read directly; `level_computation#hyp`
      would re-level it, but re-leveling an already-leveled node is a no-op
      and yields the same `level_info`.

    Used by the `Ix` case of `level_computation#expr` to avoid the
    O(context) `scx_front` slice and the recursion into definition bodies
    on every De Bruijn reference. *)
    match h.core with
    | Flex _ ->
        ...
        | Ix n -> begin
            ...
            let e_level_info = match hyp_level_info_direct hyp with
                | Some level_info ->
                    ...
                    the context or recursing into the hypothesis. *)
                    level_info
                | None ->
